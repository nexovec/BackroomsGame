...
function Talkies.say(title, messages, config)
    ...
    local newDialog = {
        ...
        onstart = config.onstart or function(dialog)
        end,
        onmessage = config.onmessage or function(dialog, left)
        end,
        oncomplete = config.oncomplete or function(dialog)
        end,
        ...
        showOptions = function(dialog)
            return dialog.messages:len() == 1 and type(dialog.options) == "table"
        end,
...
function Talkies.draw()
    ...
    if currentDialog:showOptions() and currentMessage.complete then
        if currentDialog.inlineOptions then
            ...
            love.graphics.print(currentDialog.optionCharacter .. " ", textX + currentDialog.padding,
...
function Talkies.prevOption()
    local currentDialog = Talkies.dialogs:peek()
    if currentDialog == nil or not currentDialog:showOptions() then
        return
    end
